...
def params(info):
    info.param_add_value('gridsim.mode', sps_info['mode'])
    info.param_group('gridsim.sps', label='SPS Parameters',
                     active='gridsim.mode', active_value=['SPS'], glob=True)
    info.param('gridsim.sps.v_nom', label='EUT nominal voltage for all 3 phases', default=230.0)
    info.param('gridsim.sps.v_max', label='Max Voltage', default=270.0)
    info.param('gridsim.sps.i_max', label='Max Current', default=150.0)
    info.param('gridsim.sps.freq',  label='Frequency',   default=50.0)

    info.param('gridsim.sps.comm', label='Communications Interface', default='VISA',
               values=['Serial', 'GPIB', 'VISA'])

    info.param('gridsim.sps.serial_port', label='Serial Port',
               active='gridsim.sps.comm', active_value=['Serial'], default='com1')

    info.param('gridsim.sps.gpib_bus_address', label='GPIB Bus Address',
               active='gridsim.sps.comm', active_value=['GPIB'], default=6)
    info.param('gridsim.sps.gpib_board', label='GPIB Board Number',
               active='gridsim.sps.comm', active_value=['GPIB'], default=0)

    info.param('gridsim.sps.visa_device', label='VISA Device String', active='gridsim.sps.comm',
               active_value=['VISA'], default='GPIB0::6::INSTR')
    # info.param('gridsim.sps.visa_path', label='VISA Module Path', active='gridsim.sps.comm',
